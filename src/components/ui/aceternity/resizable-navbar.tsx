...
export const NavbarLogo = ({ visible }: { visible?: boolean }) => {
  return (
    <div className={`flex items-center ml-0 mr-1 ${!visible ? 'opacity-0 pointer-events-none' : ''}`}>
      ...
      >
        <span className="text-xl bg-gradient-to-r from-emerald-500 to-emerald-900 dark:from-emerald-300 dark:to-emerald-600 bg-clip-text text-transparent relative z-10">Vardaan</span>
        <span className="hidden xl:inline text-gray-900 dark:text-[var(--text)] relative z-10"> Sharma</span>
      </a>
    </div>
  );
};
