...
const lcpPreloadSrc = "/assets/pfp-800w.webp";
...
const FloatingImage: React.FC = () => {
  ...
  const imgCommon = {
    ...
    alt: "Vardaan Sharma - Software Developer",
    width: 800,
    height: 1390,
    decoding: "async" as const,
    // Always apply rounded-3xl for both desktop and mobile
    className: "absolute inset-0 no-image-save w-60 h-[26rem] xl:w-64 xl:h-[28rem] object-cover object-top rounded-3xl transition-opacity duration-500 ease-in-out",
    style: { backfaceVisibility: "hidden" as const },
  } as const;

  const spacer = <div className="w-60 h-[26rem] xl:w-64 xl:h-[28rem]" aria-hidden="true" />;
